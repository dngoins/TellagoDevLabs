...
namespace Tellago.BizTalk.REST.Tests
{
    ...
    public class ModelFixture
    {
            ...
            get
            {
                if (ctx == null)
                {
                    ctx = new BTSModel.BTSManagementServiceContext(
                                      new Uri("http://localhost:8080/BizTalkManagementService.svc"));
                }
                return ctx;
            }
        }
        ...
        {
            var hostinstances = Context.HostInstances.Where(i => i.State == "Stopped");
        ...
        {
            var app = Context.Applications.Expand("ReceiveLocations").Where(a => a.Name.Equals("Tyco.TSP.eSales")).FirstOrDefault();
        ...
        [TestMethod]
        public void ShouldGetReceiveLocations()
        {
            ...
            if (app != null)
            {
                foreach (ReceiveLocation rl in app.ReceiveLocations)
                {
                    Console.WriteLine("{0} -> {1}d", rl.Name, rl.Enable.ToString());
                }
            }
        }
    }
